...
class WebAppTests(unittest.TestCase):
    def test_index_supports_localized_ui(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict(
                ...
                response_fi = client.get("/?lang=fi")

            self.assertEqual(response_en.status_code, 200)
            self.assertIn("Prediction history", response_en.text)
            self.assertIn("Language", response_en.text)
            self.assertIn("svg", response_en.text)

            self.assertEqual(response_fi.status_code, 200)
            self.assertIn("Ennustehistoria", response_fi.text)
            self.assertIn("Kieli", response_fi.text)
